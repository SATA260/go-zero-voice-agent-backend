...
func (m *defaultUserAuthModel) FindOneByAuthTypeAuthKey(ctx context.Context, authType string, authKey string) (*UserAuth, error) {
	var resp UserAuth
	query := fmt.Sprintf("select %s from %s where auth_type = $1 and auth_key = $2 limit 1", userAuthRows, m.table)
...
func (m *defaultUserAuthModel) FindOneByUserIdAuthType(ctx context.Context, userId int64, authType string) (*UserAuth, error) {
	var resp UserAuth
	query := fmt.Sprintf("select %s from %s where user_id = $1 and auth_type = $2 limit 1", userAuthRows, m.table)
...
func (m *defaultUserAuthModel) Insert(ctx context.Context, data *UserAuth) (sql.Result, error) {
	query := fmt.Sprintf("insert into %s (%s) values ($1, $2, $3, $4, $5, $6, $7)", m.table, userAuthRowsExpectAutoSet)
	ret, err := m.conn.ExecCtx(ctx, query, data.Id, data.DeleteTime, data.DelState, data.Version, data.UserId, data.AuthKey, data.AuthType)
	return ret, err
}
...
func (m *defaultUserAuthModel) Update(ctx context.Context, newData *UserAuth) error {
	...
	_, err := m.conn.ExecCtx(ctx, query, newData.Id, newData.DeleteTime, newData.DelState, newData.Version, newData.UserId, newData.AuthKey, newData.AuthType)
	return err
}
