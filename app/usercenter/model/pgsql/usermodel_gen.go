...
func (m *defaultUserModel) Insert(ctx context.Context, data *User) (sql.Result, error) {
	...
	ret, err := m.conn.ExecCtx(ctx, query, data.Id, data.Name, data.Email, data.Password, data.Avatar, data.Info)
	return ret, err
}

func (m *defaultUserModel) Update(ctx context.Context, data *User) error {
	query := fmt.Sprintf("update %s set %s where id = $1", m.table, userRowsWithPlaceHolder)
	_, err := m.conn.ExecCtx(ctx, query, data.Id, data.Name, data.Email, data.Password, data.Avatar, data.Info)
	return err
}
