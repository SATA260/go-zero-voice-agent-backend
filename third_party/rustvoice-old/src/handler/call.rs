...
// SIP dialog functionality removed - keeping only WebRTC and WebSocket support
...
impl ActiveCall {
    ...
    async fn do_hangup(&self, reason: Option<String>, initiator: Option<String>) -> Result<()> {
        ...
        // SIP dialog hangup removed - only WebRTC and WebSocket calls supported
        Ok(())
    }
    ...
    pub async fn cleanup(&self) -> Result<()> {
        ...
        // SIP dialog cleanup removed - only WebRTC and WebSocket calls supported
...
// SIP event loop removed - only WebRTC and WebSocket calls supported
...
pub async fn handle_call(
    ...
    // SIP dialog state channels removed - only WebRTC and WebSocket calls supported
    ...
    select! {
        ...
        // SIP event loop removed - only WebRTC and WebSocket calls supported
        r = process_call(cancel_token.clone(), call_type, call_state, session_id.clone(), ws_receiver, event_sender, state, dump_command_file) => {
            match r {
                Ok(_) => {}
                Err(e) => {
...
async fn process_call(
    ...
    let caller_track: Box<dyn Track> = match call_type {
        ...
        ActiveCallType::Sip => {
            // SIP calls are no longer supported - only WebRTC and WebSocket
            return Err(anyhow::anyhow!("SIP calls are not supported"));
        }
    };
