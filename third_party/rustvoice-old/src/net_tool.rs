...
pub async fn external_by_stun(
    ...
    let buffer = msg.raw;
    match socket.send_to(&buffer, target).await {
        Ok(_) => (),
        Err(e) => {
    ...
    let (len, _) = match tokio::time::timeout(expires, socket.recv_from(&mut buf)).await {
    ...
    info!("external address: {} -> {}", external, socket.local_addr()?);
    Ok(external)
}
...
mod tests {
    ...
    async fn test_external_by_stun() -> Result<()> {
        ...
        let external = external_by_stun(&socket, "restsend.com:3478", Duration::from_secs(5))
            .await?;
        assert_ne!(
            external,
            SocketAddr::new(IpAddr::V4(Ipv4Addr::new(0, 0, 0, 0)), 0)
        );
        info!("socket address: {}", socket.local_addr()?);
        Ok(())
    }
